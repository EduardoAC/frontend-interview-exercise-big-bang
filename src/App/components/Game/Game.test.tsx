...
describe("Game Component", () => {
  ...
  it.each(["teo", "juan"])("renders all choice buttons", (playerUsername) => {
    ...
    const section = within(
      screen.getByText(`Make your choice, ${playerUsername}`).closest("div")!,
    );
  ...
  it("displays the result after player and computer make choices", () => {
    const teoSection = within(
      screen.getByText(`Make your choice, teo`).closest("div")!,
    );
    const teoScissorsButton = teoSection.getByText("Scissors");
    fireEvent.click(teoScissorsButton);
    const juanSection = within(
      screen.getByText(`Make your choice, juan`).closest("div")!,
    );
    const juanRockButton = juanSection.getByText("Rock");
    fireEvent.click(juanRockButton);

    expect(screen.getByText("teo chose:").textContent).toBe(
      "teo chose: Scissors",
    );
    expect(screen.getByText("juan chose:").textContent).toBe(
      "juan chose: Rock",
    );
  ...
  it("updates the score after each round", () => {
    ...
    expect(sessionStorage.getItem("scoreboard")).toBe(
      JSON.stringify(updatedScored),
    );
  });
});
